...
- Stdin semantics: captured `rg` invocations inherit stdin (like canonical `rg`); to force filesystem search in piped/CI contexts, pass an explicit path (e.g. `rg-x PATTERN .`).
